...
class Splash(QWidget):
    ...
    def __init__(self, app_icon):
        ...
        # 注意: 样式表必须用 ID 选择器限定在卡片本身.
        # 不带选择器的规则会被 Qt 级联给所有子控件, 导致每个 QLabel 都被描一圈
        # 1px 边框(实测: 卡片里会凭空多出 6 条分隔线).
        self._card.setStyleSheet(
            f"QWidget#splashCard {{ background: {c['card_bg']}; border-radius: 16px;"
            f" border: 1px solid {c['line']}; }}")
        v = QVBoxLayout(self._card)
        v.setContentsMargins(0, 18, 0, 14)
        v.setSpacing(6)
        icon_label = QLabel()
        # 不要用 setScaledContents(True): 标签在 QVBoxLayout 里会横向撑满卡片,
        # 112x112 的方形图标会被拉伸成又宽又扁的一条(实测约 2.7 倍横向拉伸).
        # 用"标签定高 + 居中画原尺寸 pixmap"即可保持方形.
        icon_label.setFixedHeight(112)
