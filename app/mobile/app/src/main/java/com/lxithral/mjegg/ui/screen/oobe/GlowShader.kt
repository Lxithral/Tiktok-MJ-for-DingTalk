...
/**
 * HyperCeiler OOBE 辉光 shader —— 逐字收录自
 * `D:/desktop/App套壳设计指南/04-HyperCeiler-OOBE引导与动效实现.md` 附录 A
 * （`res/raw/glow.glsl` 381 行全文）。
 *
 * ⚠️ **照抄数值, 不要"翻译"颜色**: uColorBlack/uColorWhite 的值根本不是黑白
 * （分别是红橙 #F52828 与深蓝紫 #4D4AD7, 遗留命名）, 按名字改成黑白就是
 * "生成出来是纯白"的头号原因。唯一改动: PI/TWO_PI 加 const（AGSL 全局变量必须 const）。
 */
internal val GLOW_GLSL: String = """//------------------------Basic Params------------------------
uniform float uTime;
