...
> ✅ **当前状态：微信 / QQ / 钉钉 / 抖音 四端链路全部真机实测触发。**

### 1.0.10 — 新增抖音（手机端第四个目标应用）

- 监控 `com.ss.android.ugc.aweme` 的聊天输入框（抖音私信），功能页新增独立开关，
  诊断页按第四个目标给出逐环结论
- 无需额外适配：抖音输入框为空时 text 同样回落为占位语、清空行为被 v1.0.9 的
  `effectiveText`（text == hint ⇒ 空串）+ 空列表规则直接覆盖，开箱即通
- 真机实测：向 Yunnki 会话发送 `mj`/`mjmj` 均触发（蜘蛛侠坠落动画），
  诊断结论"链路正常（命中 1932 次，确认发送 1 次）"

### 1.0.9 — 钉钉修复补全：节点 text 属性在空时返回占位 hint

v1.0.8 只处理了**事件** text 列表里的 hint（`hint即文本` 判定），真机复测发现钉钉还有
更底层的一层：**输入框节点自身的 `text` 属性在空的时候返回的就是占位语"记录一下"**
（可编辑=true、文本可读=true）——于是：

- `onTextChanged` 里 `source文本` 优先于事件判定，"记录一下"照样抢先把候选撤销
- 80ms 轮询每拍都读到 "记录一下"，同样持续撤销候选

**修复**：新增 `effectiveText(node)` 统一归一——`text == hintText` ⇒ 返回空串
（视为输入框已清空）。事件 source 读取与轮询读取两条路径全部走它。
打字过程中若真输入了与 hint 相同的内容，候选早已被逐字符撤销，无副作用。

### 1.0.8 — 钉钉触发链路修复（清空事件里装的是占位 hint）

真机抓包定位：钉钉与微信"清空输入框"的行为**完全不同**——

- 微信：派发光标事件，text 列表是**空列表** `[]`（v1.0.7 据此修复）
- 钉钉：也派发光标事件，但 text 列表里装的是**输入框的占位提示语**（单聊自述页是
  `["记录一下"]`）！v1.0.7 的处理把它当用户文本喂状态机 → 非触发词 → **把刚登记的
  mj 候选撤销了**，触发永远出不来（诊断页表现：命中 N 次、确认 0 次）

**修复**：事件文本 == 节点自己的 `hintText` ⇒ 按"输入框已空"处理（发送确认）。
从节点现读 hint 而不是写死提示语，提示语随单聊/群聊场景变化也不怕。
该判定仍然只在已有候选时生效，不会引入新的误触发面。

### 1.0.7 — 微信触发链路修复（节点树不可见，改用事件文本判定）

真机抓包（ADB + 服务内探针）定位到微信触发不出的完整原因，与直觉相反：

1. **打字正常**：微信的 `TYPE_VIEW_TEXT_CHANGED` 事件自带文本（`event.text=["mj"]`），
   命中登记一直是好的。
2. **发送后收不到"清空"确认**：微信清空输入框时派发的是 `TYPE_VIEW_TEXT_SELECTION_CHANGED`，
   而此时它的**窗口树对无障碍完全不可见** —— `rootInActiveWindow` 只有 1 个根节点、0 个子节点
   （`uiautomator dump` 同样是空树）。于是 v1.0.6 的处理（监听光标事件后**轮询读节点**）
   永远读到 null，"变空"判据在微信上从没走通过。
3. **修复**：光标变化事件改走与 `TEXT_CHANGED` 完全相同的处理 ——
   事件自带的 `text` 列表就是信号：打字时是 `["mj"]`，清空时是 `[]`（空列表）；
   空列表 + 已有候选 ⇒ 确认"输入框被清空"（发送）。该路径会先尝试读 source 节点文本，
   读得到非空就不会误触发。不再依赖任何节点遍历。
4. **诊断结论修正**：原结论把"节点读不到文本"排在"命中过触发词"之前，导致微信明明
   靠事件文本命中过、却被误诊为"链路走不通"。已改为先报命中/确认进度。
5. 另：HyperOS 的 Greezer 会冻结只挂了无障碍服务的后台进程（广播提示 `need cached broadcast`、
   事件停发）。**装完后先打开一次 App 再去聊天**；若仍不触发，在系统设置里给 App 关掉
   电池优化 / 加锁。

### 1.0.4 — 诊断页直接给结论

原来诊断页只丢一堆原始日志，还得靠人解读。现在顶部直接给出**每个目标应用分别断在哪一环**：

| 结论 | 含义与下一步 |
|---|---|
| 没收到任何事件 | 服务没连上，或被系统/ROM 限制了后台（先查主页状态卡） |
| 收到事件但读不到输入框文本 | 该客户端没把文本暴露给无障碍，这条链路走不通 |
| 能读到文本但没命中过触发词 | 检查是否真的输入了 mj（会显示最后读到的内容，不可见字符已转义） |
| 命中过但没观察到变空 | 判据没走通（发送方式特殊） |
| 链路正常（命中 N 次 / 确认 M 次） | 那就该触发了 |

这三种情况的处理方式完全不同，所以不能只给原始日志。日志与输入框节点清单仍保留在下方。
...
| 评审意见 | 结论 |
|---|---|
| 建议补 `FLAG_LAYOUT_NO_LIMITS` | ❌ **误判**：该 flag 早已在 `EggOverlay` 里（与 `LAYOUT_IN_SCREEN` 并列） |
| `cooldownSeconds` 单位是否与命名一致 | ✅ **已自查**：默认 4、UI 显示"秒"、`fire()` 里 `coerceIn(0,60) * 1000L`，一致 |
| `event.text` 可能含 null 元素 | ✅ **采纳**：`joinToString` 会把 null 拼成字面量 `"null"` 污染文本，改为 `filterNotNull()` |
| `onInputText` 对"插入空格后不命中"的边界 | ✅ 保持现状：与桌面版 `matcher.py` 行为一致，属于规则本身，不放宽 |

另外补了一处评审和我都没提到的**真实风险**：

- **富文本输入框可能混入不可见字符**。微信/QQ 的聊天输入框是富文本编辑器，
  可能在文本里插入零宽空格（U+200B）、BOM（U+FEFF）、不换行空格（U+00A0）等，
  而 `trim()` **剥不掉它们**（`Char.isWhitespace('\u00A0')` 返回 false），
  会让"看起来就是 mj"的文本匹配失败。`Matcher.normalize()` 现在先剥掉这类字符再匹配。
  **触发词规则本身没有放宽**（`mj mj` 仍然不命中，与桌面版一致），只是清洗输入。
- **诊断日志现在会转义不可见字符**。原来 `"mj"` 和 `"mj\u200B"` 在日志里长得一模一样，
  根本查不出为什么匹配失败；现在会显示成 `"mj\u200B"`，
  并且每条文本变化都额外打印一列**归一化后**的结果，方便直接比对。
