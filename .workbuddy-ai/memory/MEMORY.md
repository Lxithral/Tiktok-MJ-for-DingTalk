...
**验证工作流（2026-09-26 用户明确要求）**：UI/交互改动装好机后**不要用 ADB 逐点验证**——
用户自己点击验证并把结果发回来。ADB 只负责装机和拉日志/截图，点击交给人。
...
**钉钉触发链路的关键事实**（2026-09-26 真机抓包定位，详见 mobile README v1.0.8/v1.0.9）：
- **钉钉输入框为空时, 节点的 text 属性返回的就是占位 hint**（自述聊天框是"记录一下"，
  可编辑=true、文本可读=true）——事件 source 文本和 80ms 轮询读到的都是它,
  直接喂状态机 = 持续撤销刚登记的候选（症状: 诊断页"命中 N 次, 确认 0 次"）。
  清空时光标事件的 text 列表里装的也是这个 hint（微信是空列表, 两家行为不同）
- v1.0.9 修复: `effectiveText(node)` 统一归一 —— text == hintText ⇒ 视为空串;
  事件 source 读取与轮询读取两条路径都必须走它（只修事件路径不够, source 文本优先级更高）
